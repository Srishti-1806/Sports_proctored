...
export default function ChatPage() {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const systemMessage = {
        ...
        If the user asks about topics OUTSIDE the sports/athletics domain (for example politics, personal legal/medical diagnosis,suicide, child abduction, etc. beyond general athlete-safe advice, programming unrelated to sports, or any other non-sports subject), you must politely refuse to answer on that topic. Respond with a short refusal such as: "I'm sorry — I can only help with sports and athletic performance topics. Please ask a question related to sports, training, or nutrition." Then invite them to return to the sports topic.
        ...
        DO NOT GIVE ANY OTHER DETAIL UNRELATED TO SPORTS`
      }
  ...
  return (
    <div className="fixed inset-0 top-16 bg-background flex overflow-hidden text-foreground">
      ...
      <motion.div
        ...
        className="fixed lg:static inset-y-0 left-0 z-50 flex flex-col w-80 bg-card border-r border-border h-full lg:transform-none!">
        {/* Sidebar Header */}
            ...
            <div className="space-y-2">
              {conversations.map((chat) => (
                <motion.div
                  ...
                  className={`p-3 rounded-xl hover:bg-card cursor-pointer transition-colors group relative ${
                    currentConversation?.id === chat.id ? 'bg-card' : ''
                  }`}
                >
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden">
        {/* Chat Header - Fixed */}
        <div className="flex items-center justify-between px-4 sm:px-6 py-4 bg-card border-b border-border shrink-0">
          {/* Left group: mobile button + title (keeps title left-aligned on phones) */}
          <div className="flex items-center">
            <button
              onClick={() => setIsSidebarOpen(true)}
              className="lg:hidden rounded-xl hover:bg-popover text-primary-deep transition-colors mr-2"
            ...
            <div className="flex items-center gap-2">
                <div className="relative">
                  <div className="w-12 h-12 rounded-2xl bg-linear-to-br from-primary-deep to-primary-bright flex items-center justify-center">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 mb-6 sm:mb-8 px-2">
                {quickActions.map((action, index) => (
                  <motion.button
                    ...
                    className="p-3 sm:p-5 rounded-2xl bg-card border border-border hover:border-primary-bright hover:shadow-lg transition-all group"
                  >
                    <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-linear-to-br from-popover to-border group-hover:from-primary-deep group-hover:to-primary-bright flex items-center justify-center mb-2 sm:mb-3 transition-all">
                      <action.icon className="w-5 h-5 sm:w-6 sm:h-6 text-primary-deep group-hover:text-white transition-colors" />
                    </div>
                    <span className="font-medium text-foreground text-xs sm:text-sm">{action.label}</span>
                  </motion.button>
                ))}
              </div>
                ...
                <div className="flex flex-wrap gap-2">
                  {suggestedPrompts.map((prompt, index) => (
                    <button
                      ...
                      className="px-3 sm:px-4 py-2 rounded-xl bg-card text-xs sm:text-sm text-primary-deep hover:bg-linear-to-r hover:from-primary-deep hover:to-primary-bright hover:text-white transition-colors"
                    >
                      {prompt}
                    </button>
                  ))}
          ...
          <div className="max-w-3xl mx-auto space-y-4 sm:space-y-6 px-2 sm:px-0">
              ...
              >
                ...
                <div className={`shrink-0 ${message.role === 'user' ? '' : ''}`}>
                  {message.role === 'assistant' ? (
                    <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-xl bg-linear-to-br from-primary-deep to-primary-bright flex items-center justify-center">
                      <Bot className="w-4 h-4 sm:w-5 sm:h-5 text-white" />
                    </div>
                  ) : (
                    <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-xl bg-linear-to-br from-primary-soft to-primary-light flex items-center justify-center text-white font-bold">
                  ...
                  <div
                    className={`inline-block max-w-[90%] sm:max-w-[85%] p-3 sm:p-4 rounded-2xl text-sm sm:text-base ${
                      message.role === 'assistant'
                        ...
                        : 'bg-linear-to-r from-primary-deep to-primary-bright text-white rounded-tr-none'
                    }`}
                  >
                    {message.role === 'assistant' ? (
                  ...
                  <div className={`flex items-center gap-2 mt-2 ${message.role === 'user' ? 'justify-end' : ''}`}>
                    ...
                    {message.role === 'assistant' && (
                      <button
                          ...
                          className="p-1 rounded hover:bg-card text-muted-foreground hover:text-primary-bright transition-colors"
                          title="Copy response"
                        >
            ...
            {isTyping && (
              ...
              >
                ...
                <div className="inline-block p-4 rounded-2xl rounded-tl-none bg-card border border-border">
                  <div className="flex items-center gap-1">
                    <motion.div
                      animate={{ scale: [1, 1.2, 1] }}
                      transition={{ duration: 0.6, repeat: Infinity, delay: 0 }}
                      className="w-2 h-2 rounded-full bg-primary-bright"
                    />
                    <motion.div
                      animate={{ scale: [1, 1.2, 1] }}
        ...
        {/* Input Area - Fixed */}
        <div className="px-3 sm:px-4 py-3 sm:py-4 bg-card border-t border-border shrink-0">
          <div className="max-w-3xl mx-auto">
            <form
              ...
              className="relative"
            >
              <div className="flex items-center gap-2 sm:gap-3 p-2 rounded-2xl bg-card/50 border border-border focus-within:border-primary-bright focus-within:ring-2 focus-within:ring-primary-bright/20 transition-all">                
                <input
                  ref={inputRef}
                  type="text"
                  value={input}
