...
export default function TeamCard({ data, onEdit, onDelete }) {
  return (
    <div className="flex items-center justify-between p-3 rounded-xl border border-[var(--color-border)] hover:border-[var(--color-primary-bright)] transition-colors group">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h4 className="font-semibold text-[var(--color-foreground)] text-sm">{data.name}</h4>
          <p className="text-xs text-[var(--color-muted-foreground,#8697C4)]">{data.startYear} - {data.endYear}</p>
        </div>
      </div>
