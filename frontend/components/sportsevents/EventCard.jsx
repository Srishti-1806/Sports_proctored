  ...
  return (
    ...
    >
      <div className="flex flex-row sm:items-start gap-3 sm:gap-4">
        {/* Date Badge */}
        <div className="shrink-0 w-14 h-14 sm:w-16 sm:h-16 rounded-2xl bg-primary flex flex-col items-center justify-center text-white mx-auto sm:mx-0 dark:bg-primary-deep dark:text-white">
          ...
          <div className="flex flex-wrap items-center gap-3 sm:gap-4 text-xs sm:text-sm text-primary-muted">
            ...
            {((event.raw?.teams_or_participants) || (event.raw?.teams)) && (
              <span className="flex items-center gap-1 whitespace-nowrap">
                <Users className="w-3.5 h-3.5 sm:w-4 sm:h-4 shrink-0" /> {event.raw?.teams_or_participants || event.raw?.teams}
              </span>
            )}

            {((event.raw?.prize_money) || event.fee) && (
              <span className="flex items-center gap-1 font-medium text-primary whitespace-nowrap">
                {event.fee || `${event.raw?.prize_money || ''}${event.raw?.prize_currency ? ' ' + event.raw.prize_currency : ''}`}
              </span>
            )}
          </div>
        </div>
